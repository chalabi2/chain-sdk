...
func TestV2_1_Parse_IP(t *testing.T) {
	sdl1, err := ReadFile("_testdata/legacy/deployment-v2.1-ip-endpoint.yaml")
	require.NoError(t, err)
...
func TestV2_1_Parse_SharedIP(t *testing.T) {
	...
	sdl1, err := ReadFile("_testdata/legacy/deployment-v2.1-shared-ip-endpoint.yaml")
	require.NoError(t, err)
	...
	resource := resources[0]
	ipEndpoint1 := findIPEndpoint(t, resource.Endpoints, 1)
	require.Greater(t, ipEndpoint1.SequenceNumber, uint32(0))

	ipEndpoint2 := findIPEndpoint(t, resource.Endpoints, 2)
...
func TestV2_1_Parse_MultipleIP(t *testing.T) {
	...
	sdl1, err := ReadFile("_testdata/legacy/deployment-v2.1-multi-ip-endpoint.yaml")
	require.NoError(t, err)
...
func TestV2_1_Parse_MultipleGroupsIP(t *testing.T) {
	...
	sdl1, err := ReadFile("_testdata/legacy/deployment-v2.1-multi-groups-ip-endpoint.yaml")
	require.NoError(t, err)
